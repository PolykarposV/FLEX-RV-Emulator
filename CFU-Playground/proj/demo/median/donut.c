...
int compare_integers(const void* a, const void* b) {
    return (*(int*)a - *(int*)b); // Returns negative if a < b, 0 if a == b, positive if a > b
}
...
void donut() { //32 bits
    ...
    // Sort the array using qsort (with inline comparison function)
    qsort(arr_32bit, size, sizeof(int), compare_integers);

    // Compute median as an integer
    if (size % 2 != 0) {
        median = arr_32bit[size / 2];  // Odd case
    } else {
        median = (arr_32bit[size / 2 - 1] + arr_32bit[size / 2]) / 2;  // Even case (truncated integer division)
    }
