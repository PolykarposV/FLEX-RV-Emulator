...
void donut() { //32 bits
    ...
    for (int i = 0; i < 128; i++) {
        if (mean > 0 && INT_MAX - mean < arr_32bit[i]) {
            mean = INT_MAX;  // Clamp to INT_MAX if overflow would occur
        } else if (mean < 0 && INT_MIN - mean > arr_32bit[i]) {
            mean = INT_MIN;  // Clamp to INT_MIN if underflow would occur
        } else
            mean += arr_32bit[i];
    }
